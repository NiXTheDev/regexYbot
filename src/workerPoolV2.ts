...
/**
 * Configuration options for WorkerPoolV2
 */
export interface WorkerPoolV2Config {
...
/**
 * WorkerPoolV2 - Dynamic worker pool with lazy initialization and auto-scaling
 *
 * Features:
 ...
 * - Maintains backward compatibility with WorkerPool API
 */
export class WorkerPoolV2 implements IWorkerPool {
	private config: WorkerPoolV2Config;
	...
	constructor(config: WorkerPoolV2Config) {
		...
		logger.info(`WorkerPoolV2 initialized with ${this.workers.size} workers`);
	}
	...
	public async shutdown(options?: {
		...
		logger.info(
			`Shutting down WorkerPoolV2${drainTasks ? " (with task draining)" : ""}...`,
		);
		...
		logger.info("WorkerPoolV2 shut down complete");
	}
}
