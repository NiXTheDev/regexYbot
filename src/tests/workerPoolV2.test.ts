...
// Skip all tests if WorkerPoolV2 is not enabled
const isV2Enabled = CONFIG.WORKER_POOL_V2_ENABLED;
const describeV2 = isV2Enabled ? describe : describe.skip;

describeV2("WorkerPoolV2", () => {
	let pool: WorkerPoolV2;

	beforeEach(() => {
		pool = new WorkerPoolV2({
			maxWorkers: 4,
			minWorkers: 1,
			initialWorkers: 1,
			taskTimeoutMs: 5000,
	...
	test("respects max workers limit", async () => {
		const limitedPool = new WorkerPoolV2({
			maxWorkers: 2,
			minWorkers: 1,
			initialWorkers: 1,
			taskTimeoutMs: 5000,
	...
	test("scales past maxWorkers during shutdown with draining", async () => {
		...
		const drainPool = new WorkerPoolV2({
			maxWorkers: 2,
			minWorkers: 1,
			initialWorkers: 1,
			taskTimeoutMs: 5000,
